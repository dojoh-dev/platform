...
const handleRoute = (
	...
	try {
		if (route) {
			console.debug('History mode:', history.type);
...
const injectMetadata = (metadata?: ComponentMetadata) => {
	if (metadata) {
		if (metadata.title) document.title = metadata.title;

		if (metadata.head) {
			metadata.head.forEach((element: HeadElement) => {
				const el = document.createElement(element.tag);
				if (element.attributes) {
					Object.entries(element.attributes).forEach(
						([key, value]) => {
...
const collectRouteContent = async (route: XRoute): Promise<string> => {
	const imports: Array<XRoute['component']> = [];
	...
	const loadedComponents = await Promise.all(
		imports.map(async (x) => {
			...
			injectMetadata(module.metadata);
