export default class Readme {
  public static transform(markdown: string): string {
    ...
    markdown = markdown.replace(/```(\w+)?\n([\s\S]*?)```/g, (_, lang, code) =>
      Readme.blockCode(code.trim(), lang || '')
    );
    ...
    // Blockquotes
    markdown = markdown.replace(/^> (.*)$/gm, (_, t) => Readme.blockquote(t));
    ...
    // Images
    markdown = markdown.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, (_, alt, url) =>
      Readme.image(alt, url)
    );

    // Links
    markdown = markdown.replace(/\[([^\]]+)\]\(([^)]+)\)/g, (_, text, url) =>
      Readme.link(text, url)
    );
